...
import { claimPlanStep, completePlanStep, failPlanStep, PlanStepStateError, releaseExpiredLeases } from "../src/application/plan-step-state.ts";
...
describe("plan step execution state", () => {
	it("claims with a stable idempotency key and unlocks dependents only after completion", () => {
		...
		expect(running.steps[0]).toMatchObject({ status: "running", attemptCount: 1, idempotencyKey: "plan-1:read:hash-read" });
		expect(() => claimPlanStep(running, "write", now)).toThrow(new PlanStepStateError("NOT_READY"));
		const completed = completePlanStep(running, "read", { idempotencyKey: "plan-1:read:hash-read", outputRef: "artifact://read" });
	...
	it("enforces concurrency keys and only releases expired leases", () => {
		const now = new Date("2026-09-11T08:00:00.000Z");
		const initial = plan([step("first", { concurrencyKey: "canvas:3" }), step("second", { concurrencyKey: "canvas:3" })]);
	...
	it("rejects a terminal update from a different idempotency key", () => {
		...
		expect(failPlanStep(running, "read", { idempotencyKey: "plan-1:read:hash-read", errorCode: "MODEL_TIMEOUT" }).steps[0]).toMatchObject({
			status: "failed",
