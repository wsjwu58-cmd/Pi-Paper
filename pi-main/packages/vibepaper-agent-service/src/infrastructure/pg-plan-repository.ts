...
export class PgPlanRepository {
	...
	/** Links a confirmed generation task to its already leased create_task step. */
	async attachTask(input: {
		...
		return await this.mutate(input.planId, input.ownerId, (plan) => {
			...
			if (
				...
				throw new PlanRepositoryError("VERSION_CONFLICT");
			return {
				...plan,
	...
	/** Internal task callbacks advance only the plan step that owns the task. */
	async completeTaskStep(input: {
		taskId: string;
		...
		return await this.database.transaction(async (client) => {
			const result = await client.query<PlanRow>(
				`SELECT plan.id, plan.session_id, plan.version, plan.canvas_version, plan.status, plan.plan_json
				 FROM agent_plans plan JOIN agent_plan_steps step ON step.plan_id = plan.id
				 WHERE step.task_id = $1 FOR UPDATE OF plan`,
				[input.taskId],
			);
			const row = result.rows[0];
			if (!row) return undefined;
			const current = toPlan(row.plan_json, row);
			const step = current.steps.find((candidate) => candidate.taskId === input.taskId);
			if (!step?.idempotencyKey || step.status !== "running") return current;
			const next =
			...
			const updated = await client.query<{ id: string }>(
				...
				[next.version, planStatus(next), JSON.stringify(next), next.id, current.version],
			);
			if (updated.rows.length !== 1) throw new PlanRepositoryError("VERSION_CONFLICT");
			for (const candidate of next.steps) await this.updateStepProjection(client, next.id, candidate);
			return next;
		});
	}
	...
	private async insertStep(
		...
		await client.query(
			`INSERT INTO agent_plan_steps
			 (id, plan_id, step_key, tool_name, depends_on, status, input_hash, estimated_cost, effect, concurrency_key, idempotency_key, task_id, lease_until, attempt_count, output_ref, last_error)
	...
	private async updateStepProjection(
		...
		await client.query(
			`UPDATE agent_plan_steps SET status = $1, effect = $2, concurrency_key = $3, idempotency_key = $4, task_id = $5,
			 lease_until = $6, attempt_count = $7, output_ref = $8, last_error = $9
			 WHERE plan_id = $10 AND step_key = $11`,
			[
				step.status,
				step.effect ?? null,
				step.concurrencyKey ?? null,
