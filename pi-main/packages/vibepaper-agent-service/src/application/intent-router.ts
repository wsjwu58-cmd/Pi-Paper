...
export type IntentDecision = {
	...
	confidence: number;
	stage: "deterministic" | "semantic" | "clarify";
...
export function routeAgentIntent(input: IntentRouterInput): IntentDecision {
	...
	if (/(?:这个|那个|刚才|上一个|它|继续做)/.test(content)) {
		return decision("conversation", 0.4, false, false, ["引用对象不明确，需要用户澄清"]);
	}
...
export function formatIntentContext(intent: IntentDecision): string {
	if (intent.requiresClarification)
		return "意图识别未能安全消歧：先用一句自然语言澄清用户目标或引用对象；禁止调用写入、生成或计费工具。";
	...
	return `本轮意图：${intent.kind}（${intent.stage} 阶段，置信度 ${intent.confidence}）。${planInstruction} 在调用工具前必须核对：目标、引用节点、是否有生成费用以及缺失信息；语义不明确时先澄清，不猜测执行。`;
}
...
function decision(
	...
	const requiresClarification =
		kind === "conversation" && confidence < 0.6 && reasons.some((reason) => reason.includes("不明确"));
	return {
		kind,
		confidence,
		stage: requiresClarification ? "clarify" : confidence >= 0.8 ? "deterministic" : "semantic",
