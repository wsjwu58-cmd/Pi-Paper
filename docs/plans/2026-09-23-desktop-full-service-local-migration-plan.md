...
同日补充：旧 v8 项目可能已有 `params.assetId` 节点，却缺少素材引用索引，导致严格校验拒绝打开。v9 升级先保存 v8 SQLite 快照，再在事务中只补齐素材存在、类型相符且没有冲突行的旧引用；备份恢复的 v8 预检适用同一条件。真实旧项目的工作区副本从 v8 升级到 v9 后成功打开，13 个节点和 3 条素材引用保持一致；原项目仍未在副本验证阶段改写。完整桌面回归 76/76 通过。
