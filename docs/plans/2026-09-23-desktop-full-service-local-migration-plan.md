...
同日素材显示修复：用户实测本地图片已登记，但素材库缩略图和画布图片节点未显示；文件内容及引用有效，桌面开发页 CSP 未允许 `vibe://app/assets/<id>`。Main 现为图片、媒体和下载请求放行受限本地协议，并关闭可替换素材的浏览器缓存。原 `AssetLibrary` 上传入口补齐多选与逐文件结果反馈，单份失败不阻断其他导入。桌面回归在工作区临时目录运行 81/81 通过；仍需在用户可见窗口中复验图片显示。
