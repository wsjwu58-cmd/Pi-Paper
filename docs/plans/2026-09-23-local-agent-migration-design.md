...
> 状态：Agent 本地 JSONL/SQLite/Markdown 存储与项目备份/恢复原型已实现。Electron Main 现在会为活动项目启动独立 Agent Worker，切换项目、备份和退出前关闭该 Worker；受限 IPC 和 preload bridge 已支持本地会话列表与新建。当前没有会话选择 UI、模型回合执行、Tool Gateway、长期记忆检索或压缩，也未做端到端验证；现阶段只证明本地会话可由宿主管理，不代表桌面 Agent 已可对话。备份会先停掉本应用 Worker；若其他进程持有项目写锁，备份明确失败。日期：2026-09-24。配套功能契约见 [桌面版 Agent 功能规格](../specs/desktop-agent-functional-spec.md)，全服务实施顺序见 [桌面本地化方案](2026-09-23-desktop-full-service-local-migration-plan.md)。目标是单用户、项目数据与 Agent 数据均由本机文件持有，正常创作不依赖云端账户、平台点数、签到、套餐或企业服务。桌面版以根目录 `AGENTS.md` 为工程契约；旧 PRD 和 V1.0 Spec 仍描述 Web 多用户架构。
