...
| 素材 | `vibepaper-services/asset-service/.../AssetService.java` 及画布素材引用 | 原件/派生文件、引用计数、删除影响、导入导出与项目备份恢复 | 原 `AssetLibrary` 的桌面上传入口现可从本机导入 PNG/JPEG/GIF/WebP 图片和 WAV 音频，Local Core 按内容校验格式、200 MB 上限及 SHA 去重；原图片专用入口仍只接受图片。素材重命名、图片/WAV 原位替换、软删除和引用计数已接本地 Store，替换保持素材 ID 和节点引用并更新内容哈希；删除保留原文件供现有节点使用。原音频节点的“存入素材库”通过受限任务 ID 保存经校验的 WAV；每次保存创建独立音频素材，音频节点引用、删除影响和项目备份恢复已接通。MP3/M4A、视频、文本、多选导入、派生文件与完整跨模态素材能力仍未迁移。 |
