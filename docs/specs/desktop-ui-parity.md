...
截至 2026-09-25，桌面路由已使用原 `WorkspacePage`、`HistoryPage`、`CanvasPage`、`AgentPanel` 与原节点组件，画布展示可创建/打开本地项目，历史记录读取本地任务。原节点内的文本、图片、视频与本地合成任务入口已接适配；合成沿用原时间线、状态、结果历史及视频预览。原音频节点已接入 Windows SAPI 本地语音任务，保持原卡片布局、结果播放器和任务历史；原“存入素材库”操作在桌面端因音频素材导入尚未迁移而不可用。Agent 内置 Skill 列表与生成确认已接通。仍需完成参考媒体输入、视频/音频素材上传、部分图像/视频后处理、动态项目 Skill，以及同状态截图与完整端到端比对。以上均不能计为 1:1 验收通过。
