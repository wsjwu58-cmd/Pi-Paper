...
截至 2026-09-26，桌面路由已使用原 `WorkspacePage`、`HistoryPage`、`CanvasPage`、`AgentPanel` 与原节点组件，画布展示可创建/打开本地项目，历史记录读取本地任务。原节点内的文本、图片、视频与本地合成任务入口已接适配；合成沿用原时间线、状态、结果历史及视频预览。原音频节点已接入 Windows SAPI 本地语音任务，保持原卡片布局、结果播放器和任务历史；原“存入素材库”按钮可把已成功的 WAV 结果保存到本地素材库。原 `AssetLibrary` 上传按钮现可多选本机图片与 WAV/MP3，逐文件反馈结果；替换按钮仍沿用原位置。桌面 CSP 已放行受限的 `vibe://app` 媒体地址，使素材库缩略图、画布节点媒体和下载可读取本地文件；原图片专用入口保持图片语义。Agent 的单个与批量音频生成沿用原确认界面。仍需完成参考媒体输入、M4A、视频及文本素材导入、部分图像/视频后处理、动态项目 Skill，以及同状态截图与完整端到端比对。以上均不能计为 1:1 验收通过。

原节点的“添加文本参考”曾直接调用 `window.prompt`，Electron 不支持时点击会抛错；桌面运行时在原按钮上使用可输入文本的对话框，Web 路径继续调用原生提示框。节点卡片位置与操作入口保持原样；对话框外观仍需在同状态截图中对照。
