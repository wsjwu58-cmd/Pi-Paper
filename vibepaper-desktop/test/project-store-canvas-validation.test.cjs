...
test('project schema v3 migrates through v7 with a v3 backup and keeps existing canvas data', async (t) => {
  const { store, directory, project } = await openTestProject(t)
...
test('project schema v4 migrates through v7 after creating a rollback snapshot', async (t) => {
  const { store, directory, project } = await openTestProject(t)
