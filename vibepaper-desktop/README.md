...
The renderer receives only narrow project, canvas, image-import, task-history, and local-model configuration methods in the preload bridge. It has no Node integration or direct filesystem access. Canvas, asset, task persistence, and backup operations run in an Electron utility process, separate from the UI and Main lifecycle process. The task panel reads local history and can cancel queued tasks; it reports that generation execution is not connected. Users can explicitly configure and discover text models exposed by a loopback OpenAI-compatible service; discovery does not run automatically and does not accept cloud endpoints or credentials. Task execution/provider adapters, Agent worker and Tool Gateway, credential vault, Agent control-store backup, and installers remain later migration stages. Cloud task creation remains disabled until data disclosure and user authorization are wired.
