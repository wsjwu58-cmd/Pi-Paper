...
The renderer receives only narrow project, canvas, image-import, and task-history methods in the preload bridge. It has no Node integration or direct filesystem access. Canvas, asset, task persistence, and backup operations run in an Electron utility process, separate from the UI and Main lifecycle process. The task panel reads the local task history and can cancel queued tasks; it clearly reports that generation execution is not connected yet. Model catalog, task execution/provider adapters, Agent worker and Tool Gateway, credential vault, Agent control-store backup, and installers remain later migration stages. Cloud task creation remains disabled until data disclosure and user authorization are wired.
